...
const YijuPhishingSimulator = () => {
  ...
  const simulateScam = async (messagesArray) => {
    try {
      const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        body: JSON.stringify({ model: 'meta-llama/llama-4-scout-17b-16e-instruct', messages: messagesArray })
      });
  ...
  // initialize the simulation when component loads
  useEffect(() => {
    startSimulation();
...
No long paragraphs, no "Kind regards."`
    ...
    try {
      ...
      const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer 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' },
        body: JSON.stringify({ model: 'meta-llama/llama-4-scout-17b-16e-instruct', messages: initialApiMessages })
      });
      ...
      setMessages([{ sender: 'System', text: 'Error starting simulation: ' + (err.message || 'Unknown error') }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (newExchangeCount >= 3) {
        ...
        setMessages(prev => [...prev, { sender: '', text: response, isReality: true, isGood: /didn['’]t share/i.test(response) }]);
      ...
      setMessages(prev => [
        { sender: 'System', text: "Error sending message: " + (error.message || "Unknown error") }
      ]);
    } finally {
      setIsLoading(false);
    }
